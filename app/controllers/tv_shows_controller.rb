class TvShowsController < ApplicationController
  ...
  # GET /tv_shows.json
  def index
    @tv_shows = TvShow.by_name_or_channel_name(params[:search])
  end
  ...
  def update
    respond_to do |format|
      if @tv_show.update(tv_show_params)
        ...
        format.json { respond_with_bip(@tv_show) }
      else
  ...
  private
    ...
    def tv_show_params
      params.require(:tv_show).permit(:name, :show_time, :channel_id, :is_favourite)
    end
end
